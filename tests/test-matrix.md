...
| T06 | Malformed/empty payload rejected by Normalize/Validate | 🔧 | The original code defaulted missing `event_id`/`rule_id`/`agent_name` to `'unknown'` instead of rejecting the event, so an empty payload could pass validation. Normalize Alert now throws before any fallback is applied — needs a live re-run with an empty payload to confirm |
...
| T21 | HTTP 3xx not treated as a successful delivery | 🔧 | Original scripts checked `status >= 400` / `< 400`, so a redirect would be misread as success. Both scripts now require `200 <= status < 300` — needs a live re-run |
